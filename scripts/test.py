...
def test_celery_config():
    ...
    try:
        ...
        # Tasks
        task_names = [name for name in sorted(app.tasks.keys()) if not name.startswith("celery.")]
...
def test_refactored_runner():
    ...
    try:
        ...
        from backend.crew_runner import cancel_crew_job, run_crew_async, validate_crew_inputs
        ...
        valid_inputs = {"course_name": "CS 101", "book_title": "Test Book", "book_author": "Test Author"}
        assert validate_crew_inputs(valid_inputs)
        print("✅ validate_crew_inputs: valid inputs accepted")

        invalid_inputs = {}
        assert not validate_crew_inputs(invalid_inputs)
...
def test_rate_limiting():
    ...
    for _ in range(5):
        ...
        if response.status_code == 429:
            print("❌ Unexpected rate limit on health check")
            return False
...
Examples:
  ...
  python scripts/test.py --setup            Test environment and file structure
    ...
    parser.add_argument("--all", action="store_true", help="Run all tests (excluding API integration tests)")
    parser.add_argument("--celery", action="store_true", help="Test Celery configuration")
    parser.add_argument("--redis", action="store_true", help="Test Redis connection")
    parser.add_argument("--refactored", action="store_true", help="Test refactored crew runner")
    parser.add_argument("--setup", action="store_true", help="Test environment and file structure")
    parser.add_argument("--imports", action="store_true", help="Test module imports")
    parser.add_argument("--queue", action="store_true", help="Test task queue configuration")
    parser.add_argument("--api", action="store_true", help="Test API rate limiting (requires running server)")
    parser.add_argument("--rate-limit", action="store_true", help="Test rate limiting configuration")
