...
from scholar_source.tools import TOCExtractorTool, WebPageFetcherTool
...
class ScholarSource:
    ...
    def resource_discovery_agent(self) -> Agent:
        ...
        return Agent(config=agent_config, llm=model, verbose=True, tools=[SerperDevTool(), YoutubeVideoSearchTool()])
    ...
    def output_formatter_agent(self) -> Agent:
        ...
        return Agent(config=agent_config, llm=model, verbose=True)
    ...
    def resource_search_task(self) -> Task:
        return Task(config=self.tasks_config["resource_search_task"], output_file=get_crew_output_file())
    ...
    def final_output_task(self) -> Task:
        return Task(config=self.tasks_config["final_output_task"], output_file=get_crew_output_file())
